...
$(function() {
    ...
    function getRandomData(ser_id) {

        if (cdata.length) {
            cdata = cdata.slice(1);
        }

//alert(cdata.length + " " + maximum);
        for(var ii=cdata.length; ii<maximum-1; ii++)
        {
            cdata.push(0);
        }
        
	$.get("./getpipestat.pl?total", function(wdata, status){
		...
		if (lastdata == 0)
		{
		    lastdata = fds[2] - 100;
                ...
                var rate = (fds[2] - lastdata) *8;
                lastdata = fds[2];
                cdata.push(rate);

                localmax = 0;
                localsum = 0;
                for(var jj=0; jj<maximum; jj++)
                {
                    localsum += cdata[jj];
                    if (localmax < cdata[jj])
                        localmax = cdata[jj];
                }
            }
        });
...
//            res.push([i, i*50 / localmax]);
            res.push([i, cdata[i]])
//            res.push([i, cdata[i] *(5-ser_id)/5 *100 / localmax])
        }
        return res;        
    }
...
//        series[3].data = getRandomData(3);
        plot.setData(series);
	plot.getAxes().yaxis.options.max = localmax *1.03;
	plot.setupGrid();
        plot.draw();
    }, 50);
